...
# Helper to find latest generated image by prefix
...
# High-Definition Verified Catalog of Masterpieces
curated_ai_catalog = [
    {
...
with open("server/data/products.json", "w", encoding="utf-8") as f:
    json.dump(curated_ai_catalog, f, indent=2, ensure_ascii=False)

# Sync to client/src/data/initialProducts.ts
categories = sorted(list(set(p.get("category", "Blind Box") for p in curated_ai_catalog)))
if "All" not in categories:
    categories.insert(0, "All")
brands = sorted(list(set(p.get("brand", "Pop Mart") for p in curated_ai_catalog)))

ts_code = f'''import {{ Product }} from '../types';

export const INITIAL_PRODUCTS: Product[] = {json.dumps(curated_ai_catalog, indent=2, ensure_ascii=False)};
...
print(f"✨ Successfully integrated {len(curated_ai_catalog)} AI-generated master studio products into catalog!")
